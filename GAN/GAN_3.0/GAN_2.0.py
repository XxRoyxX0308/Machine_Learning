...
def build_generator(z_dim):
    ...
    model.add(Conv2DTranspose(512, kernel_size=4, strides=4, padding='same', use_bias=False))
...
def build_discriminator(img_shape):
    ...
    model.add(Conv2D(64, kernel_size=4, strides=2, input_shape=img_shape, padding='same', use_bias=False))
    model.add(BatchNormalization())
    ...
    model.add(Conv2D(1, kernel_size=4, strides=1, padding='same', use_bias=False))
...
##        for i in range(8):
##            for j in range(8):
##                axs[i, j].imshow(fake_images[cnt, :, :, 0], cmap='Greys_r')
##                axs[i, j].axis('off')
##                cnt += 1
##        plt.show()
##
##fixed_latent = np.random.normal(0, 1, (64, z_dim))
...
##        g_loss = gan.train_on_batch(z, real)
        ...
        if (iteration + 1) % 10000 == 0:
            generator.save("generator_size64_"+str(iteration + 1 + times)+"_model.h5")
            discriminator.save("discriminator_size64_"+str(iteration + 1 + times)+"_model.h5")
...
####                  (iteration + 1, disc_loss, 100.0 , gen_loss))
##            print(iteration + 1)
##
...
def sample_images(epochs):
    ...
    for i in range(image_grid_rows):
        for j in range(image_grid_columns):
            ...
            ax[i,j].axis('off')
    plt.savefig("model_size64_save/animefaces_"+str(batch_size)+"/animefaces_"+str(epochs + times)+".png")
    #plt.show()
